...
$sql = mysqli_query($server, "SELECT * from data order by id_d asc "); 
$i=1;
...
<?php $i++; } ?>
  ...
  if (isset($_POST['import'])) {
   ...
   mysqli_query($server, "DELETE FROM data") or die (mysqli_error($server));
    ...
    //$ekst = explode(".", $file);
    //$fileName = "putri-".round(microtime(true)).".".end($ekst);
    ...
    foreach ($Reader as $Key => $Row)
    {
      ...
      if ($Key < 1) continue;     
      $query = mysqli_query($server, "INSERT INTO data (kelompok , pm10 , so2 , co , o3 , no , ispu , tanggal) VALUES ('".$Row[0]."', '".$Row[1]."','".$Row[2]."','".$Row[3]."','".$Row[4]."','".$Row[5]."','".$Row[6]."',NOW())");
    }
    if ($query) {
